...
namespace Blizzard.Net.Warcraft3.Statistics
{
    ...
    public unsafe struct PlayerInfo
    {
        ...
        public Span<HeroInfo> Heroes
        {
            get
            {
                fixed (byte* pHeroes = this.heroes)
                {
                    return new Span<HeroInfo>(pHeroes, (int)this.heroCount);
                }
            }
        }

        public Span<StructureInfo> Structures
        {
            get
            {
                fixed (byte* pStructures = this.structures)
                {
                    return new Span<StructureInfo>(pStructures, (int)this.structureCount);
                }
            }
        }

        public Span<UpgradeInfo> Upgrades
        {
            get
            {
                fixed (byte* pUpgrades = this.upgrades)
                {
                    return new Span<UpgradeInfo>(pUpgrades, (int)this.upgradeCount);
                }
            }
        }

        public Span<UnitInfo> Units
        {
            get
            {
                fixed (byte* pUnits = this.units)
                {
                    return new Span<UnitInfo>(pUnits, (int)this.unitCount);
                }
            }
        }

        public Span<BuildQueueInfo> UnitsInQueue
        {
            get
            {
                fixed (byte* pUnitsInQueue = this.unitsInQueue)
                {
                    return new Span<BuildQueueInfo>(pUnitsInQueue, (int)this.unitsInQueueCount);
                }
            }
        }
    }
